...
- **useResponsesApiWebSocket:** When `true`, Responses API requests use Copilot's websocket transport for models that advertise `ws:/responses`; models that only advertise `/responses` continue to use HTTP. Requests that replay encrypted `reasoning` or `compaction` history automatically use HTTP so the gateway can recover from stale encrypted carriers before any stream reaches the client. On Copilot's recoverable `invalid_request_body` response, the gateway retries once without those opaque carriers and remembers only their SHA-256 fingerprints in `invalid-encrypted-content.json`; encrypted content is never written to that file. Set to `false` to disable websocket routing and use HTTP `/responses` whenever the selected model supports it. Defaults to `true`. If the Responses API WebSocket gets closed, it is usually caused by your own network. If you are using a VPN, try switching to a different node.
